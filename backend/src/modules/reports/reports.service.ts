...
export class ReportsService {
  ...
  async generate(generateDto: GenerateReportDto, userId: string): Promise<Report> {
    ...
    const reportName = generateDto.name || 
  ...
  calculateMetrics(executions: TestExecution[]): object {
    ...
    const executionsByStatus = {
      ...
      [ExecutionStatus.PENDING]: executions.filter(e => e.status === ExecutionStatus.PENDING).length,
      [ExecutionStatus.RUNNING]: executions.filter(e => e.status === ExecutionStatus.RUNNING).length,
    };

    const executionsByTrigger = executions.reduce((acc, exec) => {
      acc[exec.trigger] = (acc[exec.trigger] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const executionsByBrowser = executions.reduce((acc, exec) => {
  ...
  generateSummary(executions: TestExecution[]): object {
    const uniqueTestCases = new Set(executions.filter(e => e.testCaseId).map(e => e.testCaseId));
    const uniqueSuites = new Set(executions.filter(e => e.suiteId).map(e => e.suiteId));

    const executionsWithErrors = executions.filter(e => 
      e.status === ExecutionStatus.FAILED || e.status === ExecutionStatus.ERROR
    );

    const topFailures = executionsWithErrors
      .slice(0, 10)
      .map(e => ({
        executionId: e.id,
